...
// Validar token de MercadoPago al arrancar
if (!MP_ACCESS_TOKEN) {
    console.error('ADVERTENCIA: MP_ACCESS_TOKEN no esta configurado. /crear-pago fallara.');
} else {
...
app.post('/crear-pago', async (req, res) => {
    ...
    if (!MP_ACCESS_TOKEN) return res.status(500).json({ error: 'MP_ACCESS_TOKEN no configurado en el servidor' });
    ...
    const titulo = esMensual
      ? 'Club GlobalVet Mexico - Plan Mensual'
          : 'Club GlobalVet Mexico - Plan Anual';
           ...
           console.log(`📦 Creando preferencia MP para: ${email} | Plan: ${plan} | Monto: $${monto}`);
      ...
      if (data.id) {
              ...
              console.error('❌ MercadoPago rechazo la preferencia:', JSON.stringify(data));
              res.status(500).json({ error: 'Error creando preferencia', detail: data });
      }
           } catch(e) {
                 console.error('❌ Error en /crear-pago:', e.message);
